...
after container being shut down.  You can obtain a shell into a running docker
container by running these commands in a new terminal:

        # get the container ID from:
        docker ps --filter "publish=8888" --format '{{.ID}}\t{{.Image}}\t{{.Ports}}'

        # get a shell with this:
